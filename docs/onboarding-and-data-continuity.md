...
- 批量照片必须先建立逐文件媒体清单，再命名；每张照片要绑定源文件名、大小、修改时间或发送顺序、短校验值、asset_id、内容摘要、拟定文件名和置信度。
- 不能只根据“第 1 张、第 2 张、第 3 张”的顺序把 AI 描述套回文件名，因为不同平台可能重排文件顺序。
- 如果内容匹配不确定，使用中性文件名并标记 `待确认`，不要生成可能错配的具体场景名。
